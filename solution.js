...
app.post("/generate", async (req, res) => {
  try {
    ...
    // Save encrypted assignments in DB (assigned_child_enc column expected)
    for (let i = 0; i < users.length; i++) {
      const plainAssigned = shuffled[i];
      const enc = encrypt(plainAssigned);
      await db.query(
        "UPDATE users SET assigned_child_enc = $1 WHERE email = $2",
        [enc, users[i]]
      );
    }
  ...
  if (req.isAuthenticated()) {
    try {
      ...
      const result = await db.query(
        "SELECT assigned_child_enc FROM users WHERE email = $1",
        [email]
      );

      const enc = result.rows[0]?.assigned_child_enc;
      const childName = enc ? decrypt(enc) : null;

      res.render("secrets.ejs", { childName });
    } catch (err) {
